...
# ---------------- MODEL PERFORMANCE ---------------- #
st.markdown("## 📊 Dataset & Model Performance")
st.write("Dataset Used: Breast Cancer Wisconsin Dataset (UCI / sklearn)")
...
def create_pdf(name, age, gender, stage, clinical_stage, probability):
    ...
    symptom_list = [ListItem(Paragraph(item, styles["Normal"])) for item in info["Symptoms"]]
    elements.append(ListFlowable(symptom_list))

    elements.append(Spacer(1, 10))

    elements.append(Paragraph("Diet Recommendation", styles["Heading2"]))
    diet_list = [ListItem(Paragraph(item, styles["Normal"])) for item in info["Diet"]]
    elements.append(ListFlowable(diet_list))

    elements.append(Spacer(1, 10))

    elements.append(Paragraph("Precautions", styles["Heading2"]))
    precaution_list = [ListItem(Paragraph(item, styles["Normal"])) for item in info["Precautions"]]
    elements.append(ListFlowable(precaution_list))
...
with col2:
    ...
    if st.button("Predict Cancer Stage") and name.strip() != "":
        ...
        try:
            ...
            ax_imp.barh(feat_imp_df["Feature"], feat_imp_df["Importance"])
            ax_imp.set_facecolor('#f9fcff')
            ax_imp.invert_yaxis()
            ...
            st.info("Feature importance available only for tree-based models (like Random Forest).")
...
st.markdown("---")
st.caption("Final Year Project | Pragnasree Yellamelli | ML • SHAP • ROC • Clinical DSS")